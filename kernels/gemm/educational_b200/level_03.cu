...
/*
Level 03: shared-memory tiling.

The key change is that each thread block cooperatively stages a tile of A and a
tile of B into shared memory, reuses that data, then moves on to the next K
tile. This is the first step away from "one thread does everything" and toward
real GPU kernel design.
*/
